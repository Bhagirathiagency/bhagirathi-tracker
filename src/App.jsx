...
function StockTab({ products = [], setProducts, receiveStock, actorName = "Owner", businessId, cases = [], fixProductNamesOnCases, standardizeAllProductNames, applyCostFormula, removeSensaTRACFromNames }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: 20, background: "lightgreen", fontSize: 18, fontWeight: 700 }}>
        BISECTION TEST 1: Tool boxes rendered OK. Product list section skipped.
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={styles.filterRow}>
        {[["financial", "Financial"], ["doctors", "Doctors"], ["team", "Team"], ["stock", "Stock"], ["overview", "Overview"]].map(([key, label]) => (
          <button key={key} onClick={() => setReportSubTab(key)} style={{ ...styles.filterChip, ...(reportSubTab === key ? styles.filterChipActive : {}) }}>{label}</button>
        ))}
      </div>
