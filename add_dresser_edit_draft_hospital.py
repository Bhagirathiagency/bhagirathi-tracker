...
apply("Add knownHospitals computation",
'''  const [customProtocol, setCustomProtocol] = useState(!PROTOCOLS.includes(Number(form.protocolDays)));''',
'''  const [customProtocol, setCustomProtocol] = useState(!PROTOCOLS.includes(Number(form.protocolDays)));
  const knownHospitals = useMemo(() => {
    const set = new Set();
    cases.forEach((c) => { if (c.hospitalName && c.hospitalName.trim()) set.add(c.hospitalName.trim()); });
    return Array.from(set).sort();
  }, [cases]);''')

apply("Add hospital name autocomplete",
...
'''        {form.billTo === "Hospital" && (
          <Field label="Hospital Name">
            <input style={styles.input} list="hospital-master-list" value={form.hospitalName} onChange={(e) => set("hospitalName", e.target.value)} />
            <datalist id="hospital-master-list">
